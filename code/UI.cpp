...
void Menu::draw_gameover(Graphics& graphics, Font& font) {
  font.render_text(graphics, "GAME OVER", Vec2f(1280, 720) / 2 - Vec2f(200, 225), 5);
...
void Menu::update_gameover(Vec2f mouse, void* game_state, bool clicked) {
  if (*(i32*)game_state = 6) {
    if (Collider::point_vs_rect(mouse, start_collider)) {
      start_color = 0xff0000ff;
      if (clicked) {
        *(i32*)game_state = 1;  //PLAY
      }
    } else {
      start_color = 0xffffffff;
    }
